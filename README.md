...
[![Version](https://img.shields.io/badge/stable-v1.10-blue.svg)](https://wordpress.org/plugins/init-void-shield/)
...
- **Core honeypot engine (always on for comments)**
  ...
  4. JavaScript + headless-browser verification — a hidden token is injected after a configurable delay plus a small random jitter *(new in 1.8)*, so the exact wait can't be read from the page source and timed around, and the script flags common automation signals (`navigator.webdriver`, a zero-size browser window) from real Selenium/Puppeteer/Playwright sessions. Static crawlers, instant bots, and unmasked headless browsers all get caught; real users don't
  5. Non-Browser User-Agent detection *(new in 1.8, on by default)* — rejects submissions whose User-Agent identifies a scripted HTTP client (curl, Python requests, Go, Scrapy, and similar) rather than a real browser, catching a bot that skips JavaScript entirely and simply replays the static form fields
  6. Block REST API Comments *(optional)* — rejects comments posted directly through the `wp/v2/comments` REST endpoint, which the classic form-based layers cannot cover
...
- **Community & e-commerce integrations** *(off by default)* — one-click honeypot guard for WooCommerce (My Account registration form), bbPress (New Topic and Reply forms), and BuddyPress (registration form); each only activates if the corresponding plugin is detected active
- **Require Real User Interaction** *(off by default, strengthened in 1.8)* — also requires at least one real mouse, keyboard, touch, or scroll event before a submission is accepted, catching bots that simply wait out the JS delay instead of interacting with the page. An event now only counts once a short minimum delay has passed since the script started, so a bot can't satisfy this with one synthetic event fired the instant the page loads
- **Require Same-Site Referer** *(new in 1.8, comments only, off by default)* — rejects a comment submission whose Referer header is missing or points to a different site, catching a scripted bot that posts directly to the comment endpoint without ever loading the page. Disclosed trade-off, same spirit as Login Guard Scope: some privacy-focused browsers strip Referer even on genuine same-site submissions, so this is meant to be used alongside the other layers, not instead of them
- **Lazy Fetch (Cache-Safe Tokens)** *(off by default)* — refreshes the time token via a small same-origin `fetch()` request (plain JavaScript, no jQuery) as soon as the page truly loads, instead of relying only on the value baked in at render time. On a heavily full-page-cached site that value reflects when the page was *cached*, not when a real visitor loaded it; this fixes that gap and applies to every guarded form. Falls back to the original baked-in token if the request fails, so it can only help
- **Login Guard Scope** — scope the Login Guard to "wp-login.php only" to leave a front-end `wp_login_form()` usage (widget, modal, custom login page) completely unguarded, e.g. if that page might be served by a full-page cache; the native `wp-login.php` form stays fully protected either way
...
- **Lightweight codebase** — backend-only, no frontend assets, and the inline verification script ships with no comments in its output *(trimmed in 1.8)*, so guarded pages stay lean and the detection logic isn't spelled out in page source
...
3. On submit, the server validates:
   - User-Agent doesn't identify a known scripted HTTP client
   ...
   - *(Optional)* Referer header points to this site, if Require Same-Site Referer is enabled
...
- **JavaScript Token Delay** — milliseconds before the JS token is injected into the form (default: 1000). A small random jitter is always added on top *(new in 1.8)*, so the exact wait can't be read from the page source and timed around
...
- **Require Same-Site Referer** *(new in 1.8, off by default)* — reject a comment submission whose Referer header is missing or points to a different site; disclosed trade-off, see Features above
...
- **Login Guard Scope** — "Everywhere" (default) covers both `wp-login.php` and any front-end `wp_login_form()` usage; "wp-login.php only" leaves the front-end usage entirely unguarded (e.g. if it lives on a page a full-page cache might serve stale) using the Referer header as a heuristic to tell the two apart
...
- **Block Non-Browser User Agents** *(new in 1.8, on by default)* — reject submissions whose User-Agent identifies a known scripted HTTP client (curl, Python requests, Go, Scrapy, and similar); the signature list is filterable
- **Require Real User Interaction** *(off by default, strengthened in 1.8)* — also require at least one real mouse, keyboard, touch, or scroll event before a submission is accepted, only counted after a short minimum delay since script start
- **Lazy Fetch (Cache-Safe Tokens)** *(off by default)* — refresh the time token client-side right after the page loads instead of relying only on the value baked in at render time; recommended for sites with aggressive full-page caching
...
**Force-exempt a request from the Require Same-Site Referer check** *(new in 1.8)*, regardless of its settings-page toggle:
```php
add_filter( 'init_plugin_suite_void_shield_referer_exempt', '__return_true' );
```
...
**Customize the JS Token Delay jitter or the minimum interaction delay** *(new in 1.8)*:
```php
add_filter( 'init_plugin_suite_void_shield_js_delay_jitter_max', function ( $milliseconds ) {
    return 600; // Default: 400.
} );
add_filter( 'init_plugin_suite_void_shield_min_interaction_delay', function ( $milliseconds ) {
    return 250; // Default: 150. Only relevant if Require Real User Interaction is on.
} );
```

**Customize the list of User-Agent substrings flagged as non-browser clients** *(new in 1.8)*:
```php
